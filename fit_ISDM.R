...
np <- read_csv(paste0(data.dir, 'blue_whale_north_pacific_isdm_data.csv')) # Blue whale location data (presence/absence) and covariates
pred_data <- read_csv(paste0(data.dir,'prediction_data.csv')) # Spatial data for prediction (mean monthly climatological conditions at .25 degree resolution)

# Add indicators for data types
np <- np %>% mutate(data_type_os = if_else(data_type == 'opportunistic sighting', 1, 0),
                    data_type_surv = if_else(data_type == 'planned survey', 1, 0),
                    data_type_tag = if_else(data_type == 'tagging', 1, 0),
                    data_type_whaling = if_else(data_type == 'whaling records', 1, 0))
...
data_list_training$os_data = np %>% filter(data_type == 'opportunistic sighting',
                                           set == 'training')
data_list_training$surv_data = np %>% filter(data_type == 'planned survey',
                                             set == 'training')
data_list_training$whaling_data = np %>% filter(data_type == 'whaling records',
                                                set == 'training')
...
##################### Model fitting #####################
# NOTE: this takes a lot of time and memory
